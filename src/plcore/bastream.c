                    ...
                    RwInt32 __RWUNUSED__ size)
{
    ...
    RWSTREAMGLOBAL(streamFreeList) = RwFreeListCreateAndPreallocateSpace(sizeof(RwStream), _rwStreamFreeListBlockSize, sizeof(RwUInt32), _rwStreamFreeListPreallocBlocks, &_rwStreamFreeList);
        ...
        case rwSTREAMMEMORY:
            {
                ...
                if (!smpMem->memBlock)
                {
                    ...
                    smpMem->memBlock = (RwUInt8 *) RwMalloc(rwSTREAMMINALLOCSIZE);
                    if (!smpMem->memBlock)
                    {
