...
PalettizeImage(RwImage **image, RwInt32 depth)
{
    ...
    _rwPalQuantResolvePalette(palette, (1UL << depth), &palQuant);
                 ...
                 RwInt32 logpalsize)
{
    ...
    while (mipLevel < totalMipLevels)
    {
        _rwPalQuantAddImage(&palQuant, mipArray[mipLevel], 1.0f);
        mipLevel++;
    }
    _rwPalQuantResolvePalette(MipPalette, 1 << logpalsize, &palQuant);
